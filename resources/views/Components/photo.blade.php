<div data-bs-toggle="dropdown" aria-expanded="false">
    ...
    @if (Auth::user()->profile->photo_path == null)
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="{{Auth::user()->profile->color}}" class="bi bi-hypnotize @if($type == 'header') header-user-photo-size @elseif($type == 'profile') profile-user-photo-size @endif cursor-pointer without-profile-photo-animation" viewBox="0 0 16 16">
    ...
    @else
        ...
        <img src="{{Auth::user()->profile->photo_path}}" alt="{{Auth::user()->profile->name}}" class="@if($type == 'header') header-user-photo-size @elseif($type == 'profile') profile-user-photo-size @endif cursor-pointer rounded-circle">
